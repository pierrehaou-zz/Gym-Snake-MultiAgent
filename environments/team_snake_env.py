...
AGENT_1_COLOR = (0, 0, 255)
AGENT_2_COLOR = (255, 0, 127)
AGENT_3_COLOR = (102, 178, 255)
AGENT_4_COLOR = (255, 102, 178)
...
class SnakeEnvironment(gym.Env):
    ...
    def __init__(self, num_agents=4, num_fruits=3, dimensions=4, spacing=22, init_length=3, reward_fruit=1.0,
        ...
        if self.flatten_states:
            self.observation_space = spaces.Box(low=-1, high=7, shape=(self.arena.dimensions ** 2 * self.history,))
        else:
            self.observation_space = spaces.Box(low=-1, high=7,
        ...
        for i, p in enumerate(self.agents):
            ...
            for f_i, f in enumerate(self.fruits):
                ...
                if f[0] == p.x[0] and f[1] == p.y[0]:
                    ...
                    # Reward scheme for team 1
                    if i % 2 == 0:

                        if i == 0:

                            if self.killed[i + 2] is False:
                                rewards[i] = self.reward_fruit
                                rewards[i+2] = self.reward_fruit
                        ...
                        # if teammate is not alive just reward self
                        else:

                            rewards[i] = self.reward_fruit

                    # Reward scheme for team 2
                    else:
                        if i == 1:
            ...
            if self.arena.coll_map[p.x[0], p.y[0]] >= 2:
                ...
                # If snakes are teammates
                elif (agent_i % 2 == 0 and i % 2 == 0) or (agent_i % 2 != 0 and i % 2 != 0):
                    continue
    ...
    def _generate_obs(self, agent):
        ...
        if self.killed[agent]:
            ...
            for i, p in enumerate(self.agents):
                ...
                # In the case of teammates, need to render obs space accordingly
                if (agent % 2 == 0 and i % 2 == 0) or (agent % 2 != 0 and i % 2 != 0):

                    # Adds head
                    obs[self.agents[i].x[0]][self.agents[i].y[0]] = TEAM_MEMBER_HEAD_OBS

                    # Adds body
                    for j in range(1, self.agents[i].length):
                        obs[self.agents[i].x[j]][self.agents[i].y[j]] = TEAM_MEMBER_OBS
