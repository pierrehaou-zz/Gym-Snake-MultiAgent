...
saving_path = "models_selfplay_3"  # The path to save our PPO_implementation to.
...
def main():
    # Starting threshold. When avg reward hits this threshold, weights are copied and PPO_implementation is saved.
    threshold = -0.5
    ...
    # Copy weight of agent1 to agent3
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for i in range(num_episodes):
            ...
            # Matching agent1, agent2, agent3 for Self-Play
            for k in range(0, 100):
                episodeBuffer = [experience_buffer(), experience_buffer(), experience_buffer()]
            ...
            if len(reward_list_agent_1) > 20:
                ...
                if np.mean(reward_list_agent_1[-20:]) > np.mean(reward_list_agent_2[-20:]) and np.mean(
                        reward_list_agent_1[-20:]) > np.mean(reward_list_agent_3[-20:]):
                    print('Updating Weight...')
                    sess.run(update_weights)
                    sess.run(update_weights_agent_3)
                    saver_new_model.save(sess, saving_path + '//model-agent_' + str(i) + '.ckpt')
